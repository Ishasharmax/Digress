...
public class JsonTests {
    ...
    void test() throws IOException {
        ...
        JsonUtil.toJsonFile("src/main/java/test.json", testStory);
        Story newStory = JsonUtil.fromJsonFile("src/main/java/test.json", Story.class);
        ...
        assertEquals(2, newStory.nodeConnections.get(1).size());
        assertEquals(1, newStory.nodeConnections.get(2).size());
        assertEquals(0, newStory.nodeConnections.get(3).size());
        assertEquals(0, newStory.nodeConnections.get(4).size());
