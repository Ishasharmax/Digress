...
public class JsonTestClass {
    ...
    void story1Tests() throws IOException {
        ...
        assertThrows(IllegalArgumentException.class, ()-> story1.getNext(3));

        //adding node to child node
        Node node4 = JsonUtil.fromJsonFile("src/test/s1n4.json", Node.class);
        //getters
        assertEquals(5, node4.getId());
        assertEquals("Content for fourth node of S1", node4.getStoryContent());
        story1.addExistingNode(node4);
        story1.getCurrNode().setChild(1, "Choice One", node4);
        assertEquals(5, story1.getNext(1).getId());
        assertEquals("Content for fourth node of S1", story1.getNext(1).getStoryContent());
